...
#define CH_QUEEN_NUM_DIRECTIONS 8

static const ch_piece_methods_t CH_QUEEN_METHODS;
static const ch_vector2_t CH_QUEEN_DIRECTIONS[] = {
...
static ch_error_t ch_queen_validate_move(ch_piece_t *piece, ch_chess_t *chess,
  ...
  if (!ch_queen_check_displacement(disp, &dir)) {
...
bool ch_chess_is_position_safe_from_queen(ch_chess_t *chess,
  ...
  for (uint8_t k = 0; k < CH_QUEEN_NUM_DIRECTIONS; k++) {
    ...
    for (uint8_t s = 1; s < CH_BOARD_SIZE; s++) {
      ...
      if (piece->type == CH_PIECE_TYPE_QUEEN && piece->side != chess->turn) {
        return false;
      }
      break;
    }
