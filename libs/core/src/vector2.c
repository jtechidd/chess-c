...
ch_vector2_t ch_vector2_inv(ch_vector2_t v) {
  return ch_vector2_scalmult(v, -1);
}

ch_vector2_t ch_vector2_fliph(ch_vector2_t v) {
  return ch_vector2_make(v.i, -v.j);
}
...
bool ch_vector2_parallel(ch_vector2_t v1, ch_vector2_t v2) {
  double dot = ch_vector2_dot(v1, v2);
  double mag_mul = ch_vector2_magnitude(v1) * ch_vector2_magnitude(v2);
  return fabs(dot - mag_mul) < 0.00001;
}