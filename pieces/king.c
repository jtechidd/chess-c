...
void king_add_moves_positional(king_t *king, board_t *board, move_array_t *move_array) {
...
rook_t *king_get_rook_to_castle_with(king_t *king, board_t *board, move_castling_type_t move_castling_type) {
...
void king_add_moves_castle(king_t *king, board_t *board, move_array_t *move_array, move_castling_type_t move_castling_type) {
  ...
  for (uint8_t j = 1; j <= 2; j++) {
    ...
    if (!board_is_position_safe_to_move_to(board, king->piece.side, check_position)) {
      return;
    }
  }
