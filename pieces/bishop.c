...
bishop_t* bishop_clone(bishop_t* bishop_src) {
    bishop_t* bishop = bishop_new(bishop_src->piece.id, bishop_src->piece.side, bishop_src->piece.position);
...
move_array_t* bishop_get_positional_moves(piece_t* piece, board_t* board) {
    bishop_t* bishop;
    ...
    for (size_t k = 0; k < BISHOP_TOTAL_DIRECTIONS; k++) {
        ...
        for (uint8_t scale = 1;; scale++) {
            vector2_t position_to = vector2_add2(bishop->piece.position, vector2_scalar_multiply(direction, scale));
...
uint8_t board_is_position_get_attacked_by_bishop(board_t* board, side_t side, vector2_t position) {
    for (size_t k = 0; k < BISHOP_TOTAL_DIRECTIONS; k++) {
        vector2_t direction = BISHOP_DIRECTIONS[k];
        for (uint8_t scale = 1;; scale++) {
