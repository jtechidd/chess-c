...
move_array_t* rook_get_positional_moves(piece_t* piece, board_t* board) {
    rook_t* rook;
    ...
    for (size_t k = 0; k < ROOK_TOTAL_DIRECTIONS; k++) {
        ...
        for (uint8_t scale = 1;; scale++) {
            vector2_t position_to = vector2_add2(rook->piece.position, vector2_scalar_multiply(direction, scale));
